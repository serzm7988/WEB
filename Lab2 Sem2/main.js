...
function Show(e) {
    e.preventDefault();
    if(document.getElementById(`question`).checked == true) document.getElementsByTagName(`img`)[0].src = `adjacent.png`;
...
function Calculate(e) {
    ...
    let angle = (document.getElementById(`question`).checked) ? document.getElementById(`ang`).value : (180-document.getElementById(`ang`).value)/2;
    angle = angle * Math.PI / 180;
    const osn = document.getElementById(`osn`).value;
    ...
    if(document.getElementsByTagName(`select`)[0].options[1].selected) {
        ...
        let S = osn*H/2;
        let P = Number(a)+Number(osn)+Number(a);
        let Rad = S/(P/2);
